...
def dib_image_command(parser):
    ...
    dib_imageupdate = subc.add_parser('update',
                                      help='trigger the local rebuild of '
                                           'an image')
    dib_imageupdate.add_argument('--provider', '-p', metavar='provider-name',
                                 required=True)
    ...
    dib_imagelogs = subc.add_parser('logs',
                                    help='download the build logs of a dib'
                                         'image')
    dib_imagelogs.add_argument('--image', '-i', metavar='dib-image-name',
...
def dib_image_action(args, base_url):
    ...
    if args.subcommand == 'upload':
        x = (True and getattr(args, 'image') or '',
             True and getattr(args, 'provider') or '')
        url = build_url(base_url,
